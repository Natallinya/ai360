...
const TOOLS: Tool[] = [
  {
    name: 'rule_list',
    description:
      `List rule sections inside single file .cursor/rules/${CONSOLIDATED_RULES_FILENAME}.`,
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'rule_read',
    description:
      `Read .cursor/rules/${CONSOLIDATED_RULES_FILENAME} (full file) or one section by slug.`,
    inputSchema: {
      type: 'object',
      properties: {
        slug: {
  ...
  },
  {
    name: 'rule_install',
    description:
      'Add or update a rule section in ONE file .cursor/rules/custom-rules.mdc. Use when user says «Установи правило …» — title + content. Replaces section with same title if it exists.',
    inputSchema: {
      type: 'object',
      properties: {
        title: {
  ...
  switch (name) {
    ...
    case 'rule_install': {
      ...
      const replaceExisting =
        typeof args.replaceExisting === 'boolean' ? args.replaceExisting : true;

      if (!title || !content) {
        return textResult({ error: 'title and content are required' });
      }

      const { filePath, slug, action } = await installRuleSection(
        title,
        content,
        replaceExisting,
      );
      ...
      return textResult({
        ...
        message:
          `Правило «${title}» ${action === 'updated' ? 'обновлено' : 'добавлено'} в ` +
...
async function main(): Promise<void> {
  const server = new Server(
    { name: 'ai360-rules', version: '0.2.0' },
    { capabilities: { tools: {} } },
  );
