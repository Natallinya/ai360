...
export const CONSOLIDATED_RULES_FILENAME = process.env.AI360_RULES_FILE ?? 'custom-rules.mdc';
...
export function consolidatedFilePath() {
    return path.join(rulesDir(), CONSOLIDATED_RULES_FILENAME);
}
export async function ensureRulesDir() {
    const dir = rulesDir();
    await mkdir(dir, { recursive: true });
    return dir;
}
export async function ensureConsolidatedFile() {
...
export async function readConsolidatedFile() {
    const filePath = await ensureConsolidatedFile();
    return readFile(filePath, 'utf8');
}
export function parseSections(fileContent) {
...
export async function readRuleSection(slug) {
    const filePath = await ensureConsolidatedFile();
...
export async function installRuleSection(title, content, replaceExisting = true) {
    const filePath = await ensureConsolidatedFile();
    const fileContent = await readFile(filePath, 'utf8');
    const { frontmatter, preamble, sections } = parseSections(fileContent);
    const slug = slugifyTitle(title);
    const trimmedContent = content.trim();
    const existingIndex = sections.findIndex((section) => section.slug === slug || section.title === title);
    let action;
    if (existingIndex >= 0) {
        if (!replaceExisting) {
...
function rebuildConsolidatedFile(frontmatter, preamble, sections) {
